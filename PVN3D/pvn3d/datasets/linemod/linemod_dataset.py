...
class LM_Dataset():
    ...
    def __init__(self, dataset_name, cls_type="duck"):
        ...
        if dataset_name == 'train':
            ...
            print('fuse img path: ', fuse_img_pth)
            ...
            try:
                ...
                print('fuse == render')  #modified by DHZ
                # self.fuse_lst = None
            # self.all_lst = self.real_lst + self.rnd_lst + self.fuse_lst 
            self.all_lst = self.real_lst + self.fuse_lst
            # self.all_lst = self.real_lst

            # print('self.all_lst: ', self.all_lst)
        else:
    ...
    def get_normal(self, cld):
        ...
        cld = cld.astype(np.float32)
        # print('fuck you 111')
        cloud.points = open3d.Vector3dVector(cld)
        # print('fuck you 222')
        estimate_normals(cloud, search_param=open3d.geometry.KDTreeSearchParamKNN(knn=50))
        
        # n = cloud.normals
        nn = open3d.np.asarray(cloud.normals)
        # print('fuck you')
        return nn
    ...
    def get_item(self, item_name):
        try:
            if "pkl" in item_name:
            # if "fuse" in item_name:

                # path = "/home/zjunlict/dhz/NeuroRobotics/PVN3D/pvn3d/datasets/linemod/Linemod_preprocessed/"
                # item_name = path + item_name + "_info.pkl"
                ...
                data = pkl.load(open(item_name, "rb"))
                # data = {'depth':123, 'rgb':[2,3,4], 'mask':[1,2,3], 'K':12, 'RT':12, 'rnd_typ':'fuse'}
                # print('data type', type(data))
                # print('data: ', data)
                dpt = data['depth']
                rgb = data['rgb']
            ...
            # added by DHZ
            elif "suse" in item_name:
                # path = "/home/zjunlict/dhz/NeuroRobotics/PVN3D/pvn3d/datasets/linemod/Linemod_preprocessed/"
                path = '/home/zjunlict/dhz/NeuroRobotics/PVN3D/pvnet-rendering/data/LINEMOD/'
                name = path + 'ape' +  item_name + ".pkl"
                ...
                # with Image.open(os.path.join(path, item_name+"_depth.png")) as di:
                #     dpt = np.array(di)
                # with Image.open(os.path.join(path, item_name+"_mask.png")) as li:
                #     labels = np.array(li)
                #     # labels = (labels > 0).astype("uint8")
                #     labels = (labels == self.cls_id).astype("uint8")
                # with Image.open(os.path.join(path, item_name+"_rgb.jpg")) as ri:
                #     if self.add_noise:
                #         ri = self.trancolor(ri)
                #     rgb = np.array(ri)[:, :, :3]
                # # item_name = item_name[5:0]
                # item_name = item_name.strip('fuse/')
