...
from lib.loss import OFLoss, FocalLoss, CONFLoss
...
def model_fn_decorator(
    criterion, criterion_of, criterion_conf, test=False
):
    ...
    def model_fn(
        ...
        with torch.set_grad_enabled(not is_eval):
            ...
            rgb, pcld, cld_rgb_nrm, choose, kp_targ_ofst, ctr_targ_ofst, \
                cls_ids, rts, labels, kp_3ds, ctr_3ds, \
                model_points, model_kp = cu_dt

            pred_kp_of, pred_rgbd_seg, pred_ctr_of, pred_kp_conf = model(
            ...
            # print(rts[0])
            loss_kp_conf = criterion_conf(
                pred_kp_of, pred_kp_conf, pcld[0], model_points, model_kp, rts[0][0][:, :3], rts[0][0][:, 3],
            ).sum()
            w = [2.0, 1.0, 1.0]
            loss = loss_rgbd_seg * w[0] + loss_kp_of * w[1] + \
                   loss_ctr_of * w[2] + loss_kp_conf
            ...
            if is_test:
                teval.eval_pose_parallel(
                    pcld, rgb, labels, pred_ctr_of, ctr_targ_ofst,
                    labels, epoch, cls_ids, rts, pred_kp_of, pred_kp_conf,
                    min_cnt=1, use_p2d=False, use_ctr_clus_flter=False,
                    use_ctr=True, ds_type="linemod", obj_id=obj_id
                )
        ...
        return modelreturn(
            (pred_kp_of, pred_rgbd_seg, pred_ctr_of), loss,
            {
                ...
                "loss_kp_conf": loss_kp_conf.item(),
