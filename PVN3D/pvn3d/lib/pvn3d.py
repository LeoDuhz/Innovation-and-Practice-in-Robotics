...
class PVN3D(nn.Module):
    ...
    def forward(self, pointcloud, rgb, choose):
        ...
        """
        # print('dhz debug: rgb type: ', type(rgb))
        # print('dhz debug: rgb.size(): ', rgb.size())
        out_rgb, rgb_seg = self.cnn(rgb)
        # print('dhz debug: out_rgb type: ', type(out_rgb))
        bs, di, _, _ = out_rgb.size()
        # print('dhz debug: out_rgb.size(): ', out_rgb.size())
        # print('dhz debug: bs, di = ', bs, di)
        ...
        pred_kp_conf = self.KpConf_layer(rgbd_feature).view(
            bs, self.num_kps, 1, N
        )
        pred_kp_conf = pred_kp_conf.permute(0, 1, 3, 2).contiguous()

        return pred_kp_of, pred_rgbd_seg, pred_ctr_of, pred_kp_conf
