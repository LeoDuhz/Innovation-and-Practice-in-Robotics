...
def cal_frame_poses_lm(
    pcld, mask, ctr_of, pred_kp_of, pred_kp_conf, use_ctr, n_cls, use_ctr_clus_flter, obj_id
):
    n_kps, n_pts, _ = pred_kp_of.size()
    pred_ctr = pcld - ctr_of[0]
    pred_kp = pcld.view(1, n_pts, 3).repeat(n_kps, 1, 1) - pred_kp_of
    
    # for i in range(pred_kp[0].size()[0]):
    #     print(pred_kp[0][i])
    # print(pred_kp.size(), pred_kp_conf.size())
    # print(torch.sum(confidence, 1))
    # print("pred_kp", pred_kp)
    # print("confidence", confidence)
    # print("pred_kp_new",pred_kp_new)

    radius=0.08
    if use_ctr:
        cls_kps = torch.zeros(n_cls, n_kps+1, 3).cuda()
        cls_kps_new = torch.zeros(n_cls, n_kps+1, 3).cuda()
    else:
        cls_kps = torch.zeros(n_cls, n_kps, 3).cuda()
    ...
    if cls_msk.sum() < 1:
        ...
        if ctr_labels.sum() < 1:
            ctr_labels[0] = 1
        if use_ctr:
            cls_kps[cls_id, n_kps, :] = cls_kps_new[cls_id, n_kps, :] = ctr
        ...
        pred_kp_conf = pred_kp_conf[:, cls_msk, :]
        confidence = torch.empty(n_kps, pred_kp_conf.size()[1], 1).cuda()
        for i in range(n_kps):
            confidence[i] = pred_kp_conf[i] / torch.sum(0.00001 + pred_kp_conf[i])
        pred_kp_new = torch.sum(confidence * cls_voted_kps, 1)
        # print("confidence", pred_kp_conf[:, cls_msk, :].size())
        # print("cls_voted_kps", cls_voted_kps.size())
        # for ikp, kps3d in enumerate(in_pred_kp):
        #     cls_kps[cls_id, ikp, :], _ = ms.fit(kps3d)
        cls_kps_new[cls_id, :n_kps, :] = pred_kp_new
        # print(cls_kps[cls_id], cls_kps_new[cls_id])
        ...
        pred_RT = bs_utils_lm.best_fit_transform(
            mesh_kps.contiguous().cpu().numpy(),
            cls_kps_new[cls_id].squeeze().contiguous().cpu().numpy()
        )
        pred_pose_lst.append(pred_RT)
...
def eval_one_frame_pose_lm(
    ...
    pcld, mask, ctr_of, pred_kp_of, pred_kp_conf, RTs, cls_ids, use_ctr, n_cls, \
        min_cnt, use_ctr_clus_flter, label, epoch, ibs, obj_id = item
    pred_pose_lst = cal_frame_poses_lm(
        pcld, mask, ctr_of, pred_kp_of, pred_kp_conf, use_ctr, n_cls, use_ctr_clus_flter,
        obj_id
    )
...
class TorchEval():
    ...
    def eval_pose_parallel(
        ...
        cls_ids, RTs, pred_kp_ofs, pred_kp_conf, min_cnt=20, merge_clus=False, bbox=False, ds='YCB',
        ...
        if ds_type == "ycb":
            ...
            data_gen = zip(
                pclds, masks, pred_ctr_ofs, pred_kp_ofs, pred_kp_conf, RTs,
                cls_ids, use_ctr_lst, n_cls_lst, min_cnt_lst, use_ctr_clus_flter_lst,
