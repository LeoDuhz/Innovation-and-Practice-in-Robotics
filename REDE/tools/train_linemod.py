...
def main():
    ...
    # dataset = PoseDataset_linemod('train', opt.num_points, opt.dataset_root, True, False, False, opt.obj_id)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=True, num_workers=opt.workers)
    print("testdataset")
    test_dataset = PoseDataset_linemod('test', opt.num_points, opt.dataset_root, True, False, False, opt.obj_id)
    print("after testdataset")
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for j, data in enumerate(testdataloader, 0):
            ...
            if len(points.size()) == 2:
                print('breakdown in normal test')
