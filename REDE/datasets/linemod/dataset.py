...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, root, real, render, fuse, obj_id):
        ...
        if real:
            ...
            while 1:
                ...
                if self.mode == 'eval':
                    # self.list_label.append('{0}/our_seg_result/{1}/our_mask/{2}.png'.format(self.root, '%02d' % self.obj_id, input_line))
                    self.list_label.append('{0}/segnet_results/{1}_label/{2}_label.png'.format(self.root, '%02d' % self.obj_id, input_line))
                else:
                    self.list_label.append('{0}/data/{1}/our_mask/{2}.png'.format(self.root, '%02d' % self.obj_id, input_line))
    ...
    def __getitem__(self, index):
        ...
        # elif self.list_rgb[index].find('fuckuse') != -1:
            img = Image.open(self.list_rgb[index])
            ori_img = np.array(img)
            depth = np.array(Image.open(self.list_depth[index]))
            label = np.array(Image.open(self.list_label[index]))
            ...
            if len(label.shape) == 2:
                mask_label = ma.getmaskarray(ma.masked_equal(label, np.array(1)))#here
            else:
            ...
            #     for j in range(640):
            #         if mask_label[i][j] != 0:
            #             # print(mask_label[i][j])
            #             flag = True
            # if flag:
            #     print("mask_label is normal")
            # else:
            #     print("fuuuuuck")
